...
async function api(method, path, body) {
  // Mock responses keyed by path pattern
  if (path === '/auth/login') return { token: 'demo', user: { username: 'demo_user', role: 'student' } };
  if (path === '/auth/register') return { token: 'demo', user: { username: body.username, role: body.role } };
  if (path.startsWith('/timer/stats')) return { overall: { completed_sessions: 3, total_hours: 2.1 } };
  if (path === '/goals') return { goals: [{ id:1, title:'Final Exam', description:'Study hard!', target_date:'2025-06-01', days_remaining:14, completed:false }] };
  if (path === '/notes/inbox') return { notes: [{ emoji:'🌸', message:'You got this!', from_username:'supporter', created_at: new Date().toISOString() }] };
  if (path === '/timer/sessions') return { sessions: [] };
  if (path === '/mood/history') return { entries: [] };
  if (path === '/friends') return { friends: [] };
  if (path === '/friends/requests') return { requests: [] };
  // For POST/PATCH/DELETE actions
  return { message: '✨ (Demo mode — changes not saved)' };
}
